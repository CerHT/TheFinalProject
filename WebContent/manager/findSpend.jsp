		...
		<form name="form" >
			<table>
				<tr>
					...
					<td>is_use_cash</td>
					<td>cash</td>
					<td>business_request</td>
				</tr>
				<c:forEach items="${list }" var="user" varStatus="status">
					<tr id="<c:out  value="${user.id}"/>">
						<td><input value="${user.id.time}" readonly="readonly" /></td>
						<td><input name="list[${status.index}].u_id"
							value="${user.id.u_id}" readonly="readonly" /></td>
						<td><input name="list[${status.index}].c_id"
							value="${user.c_id}" readonly="readonly" /></td>
						<td><input name="list[${status.index}].money"
							value="${user.money}" readonly="readonly" /></td>
							
						<td><input name="list[${status.index}].recommend"
							value="${user.recommend}" readonly="readonly" /></td>	
						...
						<td><input name="list[${status.index}].business_request"
							value="${user.business_request}" readonly="readonly" /></td>								
					</tr>
				</c:forEach>
			</table>
		</form>
		...
		<table>
			<tr>
				<td colspan="6" align="center" bgcolor="#5BA8DE">共${page.totalRecords}条记录
					...
					href="<%=request.getContextPath()%>/manager/findSpend?pageNo=${page.topPageNo}"><input
						type="button" name="fristPage" value="首页" /></a> <c:choose>
						<c:when test="${page.pageNo!=1}">
							<a
								href="<%=request.getContextPath()%>/manager/findSpend?pageNo=${page.previousPageNo }"><input
						...
						<c:when test="${page.pageNo != page.totalPages}">
							<a
								href="<%=request.getContextPath()%>/manager/findSpend?pageNo=${page.nextPageNo }"><input
					...
					href="<%=request.getContextPath()%>/manager/findSpend?pageNo=${page.bottomPageNo}"><input
