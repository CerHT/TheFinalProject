			...
			<table>
				...
				<c:forEach items="${list }" var="user" varStatus="status">
					<tr id="<c:out  value="${user.c_id}"/>">
						...
						<td><input type="botton" value="购买" /></td>
					</tr>
				</c:forEach>
			</table>
		</form>
		...
		<table>
			<tr>
				<td colspan="6" align="center" bgcolor="#5BA8DE">共${page.totalRecords}条记录
					...
					href="<%=request.getContextPath()%>/platform/commodity?pageNo=${page.topPageNo}"><input
						type="button" name="fristPage" value="首页" /></a> <c:choose>
						<c:when test="${page.pageNo!=1}">
							<a
								href="<%=request.getContextPath()%>/platform/commodity?pageNo=${page.previousPageNo }"><input
						...
						<c:when test="${page.pageNo != page.totalPages}">
							<a
								href="<%=request.getContextPath()%>/platform/commodity?pageNo=${page.nextPageNo }"><input
					...
					href="<%=request.getContextPath()%>/platform/commodity?pageNo=${page.bottomPageNo}"><input
